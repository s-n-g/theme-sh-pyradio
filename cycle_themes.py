...
def make_target():
    ...
    if not os.path.exists(target):
        ...
        sys.exit(1)
    target = os.path.join(target, 'cycle_theme_sh_themes.pyradio-theme')
    ''' reset the file '''
    try:
...
def do_copy_files(scr, out_file, stop, delay, th_start_count, que):
    ...
    scr.addstr(0, 0, ' Cycle PyRadio theme.sh Themes', curses.A_BOLD)
...
parser = ArgumentParser(description='Cycle through PyRadio theme.sh themes')
...
msg =  '''Please execute PyRadio now and "watch" the "cycle_theme_sh_themes" theme

To do that:
  1. Execute PyRadio
  2. Press "t" to open the "Themes Selection" window
  3. Select the "cycle_theme_sh_themes" entry
